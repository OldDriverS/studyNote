...
当链接还处于 new 状态时，说明，这条链接可能

- 从内部发往外部网络的
- 由外部网络发起的连接

当连接从 new 转换成 established 时，可能：
...
- 在状态为new时，过路由后，应即使打ct mark 标记区分连接，当前发出的口，以后一直用这个接口发出
- 在状态进一步转换成 established，过路由前，应从conntrack 中取出ct mark 设置数据包meta mark，以影响路由查找策略

# 第一个数据包，针对contrack 记录打ct mark

根据 netfilter hook 对应的几个检查点，状态为 **NEW** 的数据包

```
output => 路由查找 => postrouting => 加入目标接口发送队列
preruoting => 路由查找 => forward => postrouting => 加入目标接口发送队列
prerouting => 路由查找 => input => 发往本机
```

output 位置发现的 NEW 数据包，修改它可以影响路由查找，**适用对路由器本机分流** ，但是这个位置还没确定发送接口，不适合打 **ct mark**

forward 或者 postrouting 位置都 **适合对连接进行ct mark 打标** ，

这两个位置发现的 NEW 数据包，同时可以看见数据包从哪个网口发出，已经确定了发送接口 **oif** ，可以通过匹配 **oif** 区分标记，
...
当第一个数据包通过防火墙，状态为 new 时，已经在conntrack 建立了条目，并且打上了 ct mark 标记，那么对于有标记的连接，应该在路由以前，将ct mark标记应用到数据包的meta mark
...
在 output 处，状态为 established 的连接，是已建立的链接，它需要固定一个路线，这里应该对数据包进行mark以影响它的路由查找
...
table inet mangle {
        chain prerouting {
                ...
                ct state { established, related } meta mark set ct mark counter comment "已建立链接的ct标记恢复"
        }
